...
def train(FE, CF, MI, data_train_loader, current_lr, vis=None):
    ...
    for i, (sentences, labels, sentences_length) in enumerate(data_train_loader):
        ...
        x_prime_length = sentences_length[1:] + [sentences_length[0]]
        ...
        z = FE(sentences)
        output = CF(z, sentences_length)
        loss_target = criterion(output, labels)
        loss_jsd = -info_loss(MI, x, sentences_length, z, u, x_prime, x_prime_length)
        loss = -loss_target + 0.5*loss_jsd
        ...
        if vis is not None:
            if index == 0:
                ...
                vis.line(Y=np.array([loss_jsd]), X=np.array([index]), win='train_jsd_loss', opts={
                    'title': 'jsd loss'
                })
            else:
                ...
                vis.line(Y=np.array([loss_jsd]), X=np.array([index]), win='train_jsd_loss', update='append')
